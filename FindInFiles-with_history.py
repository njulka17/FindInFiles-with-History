...
class SearchDataBase:
    ...
    def check_db(self) -> bool:
        if os.path.isfile('search-history.db'):
            return True
        return False

    def check_tables(self) -> bool:
        tables = [i[0] for i in self.cursor.execute("SELECT name FROM sqlite_master WHERE type='table';").fetchall()]
        if tables == TABLES:
            return True
        return False

    def create_connect_db(self) -> sqlite3.Connection:
        self.connection = sqlite3.connect('search-history.db')
        return self.connection

    def create_tables(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute('CREATE TABLE IF NOT EXISTS keyword_history(id INTEGER PRIMARY KEY, keyword TEXT);')
            cursor.execute('CREATE TABLE IF NOT EXISTS path_history(id INTEGER PRIMARY KEY, path TEXT);')
            cursor.execute('CREATE TABLE IF NOT EXISTS search_time(id INTEGER PRIMARY KEY, start_time DATETIME NOT NULL'
                           ', end_time DATETIME NOT NULL);')
            cursor.execute("CREATE TABLE IF NOT EXISTS extension_history(id INTEGER PRIMARY KEY, extension TEXT);")
            self.connection.commit()
            return True
        except AttributeError or sqlite3.Error:
            return False

    def connect(self) -> sqlite3.Cursor:
        if self.check_db() is True:
            self.cursor = self.create_connect_db().cursor()
            if self.check_tables() is True:
                return self.cursor
            else:
                if self.create_tables(self.cursor) is True:
                    return self.cursor
                else:
                    print("search history not intialized: cannot create tables")
        else:
            return self.create_connect_db().cursor()

    def execute(self, statement: str, *parameters):
        result = self.cursor.execute(statement, parameters).fetchall()
        self.connection.commit()
        self.last_execution_result = result
        return result

    def get_distinct_keywords(self) -> str:
        return [i[0] for i in self.cursor.execute("SELECT DISTINCT keyword FROM keyword_history;").fetchall()]

    def get_distinct_paths(self) -> str:
        return [i[0] for i in self.cursor.execute("SELECT DISTINCT path FROM path_history;").fetchall()]

    # def get_times(self) -> str:
    #     return self.cursor.execute("SELECT DISTINCT keyword FROM keyword_history;").fetchall()

    def get_distinct_extensions(self) -> str:
        return [i[0] for i in self.cursor.execute("SELECT DISTINCT extension FROM extension_history;").fetchall()
                if i[0] not in [None, 'None']]

    def insert(self, keyword: str, path: str, start_time: datetime.datetime, end_time: datetime.datetime,
               extension=None):
        search_id = self.new_id()
        self.idlist.append(search_id)
        self.insert_keyword(search_id, keyword)
        self.insert_path(search_id, path)
        self.insert_time(search_id, start_time, end_time)
        self.insert_extension(search_id, extension)

    def insert_keyword(self, kid, keyword):
        if keyword.__contains__('"'):
            keyword = keyword.replace('"', "'")
        self.cursor.execute(f"""INSERT INTO keyword_history VALUES({kid}, "{keyword}");""")
        self.connection.commit()

    def insert_path(self, pid, path):
        if path.__contains__('"'):
            path = path.replace('"', "'")
        self.cursor.execute(f"""INSERT INTO path_history VALUES({pid}, "{path}");""")
        self.connection.commit()

    def insert_time(self, tid, start_time, end_time):
        self.cursor.execute(f"INSERT INTO search_time VALUES({tid}, '{start_time}', '{end_time}');")
        self.connection.commit()

    def insert_extension(self, eid, extension):
        self.cursor.execute(f"INSERT INTO extension_history VALUES({eid}, '{extension}');")
        self.connection.commit()

    def initialize_idlist(self) -> list:
        return [i[0] for i in self.cursor.execute('SELECT id FROM keyword_history').fetchall()]

    def new_id(self) -> str:
        rand_str = str(random.random()).lstrip('0.')
...
class ToolTip(Toplevel):
    ...
    ToolTip constructor
    """
    def __init__(self, wdgt, tooltip_font, msg=None, msgfunc=None, delay=0.5, exitdelay=4.8, follow=True):
        """
    ...
    def show(self):
        ...
        if self.visible == 2:
            self.deiconify()
            self.after(int(self.exitdelay * 1000), self.hide)
...
class AutoCompletePlaceHolderCombobox(ttk.Combobox):
    _completion_list = []
    _hits = []
    _hit_index = 0
    position = 0
...
class FindInFiles(Tk):
    ...
    def __init__(self, ez_py=None):
        ...
        self.search_history = SearchDataBase()
        self.x = None
        self.y = None
        self.tree_iid = 0
        ...
        self.search_entry.set_completion_list(self.search_history.get_distinct_keywords())
        ...
        self.extension_entry = AutoCompletePlaceHolderCombobox(self.configurations_frame, width=7, placeholder='')
        ...
        self.extension_entry.set_completion_list(self.search_history.get_distinct_extensions())
        ...
        self.folder_entry = AutoCompletePlaceHolderCombobox(self.folder_frame, placeholder='')
        self.folder_entry.insert(0, self.rootdir)
        self.folder_entry.set_completion_list(self.search_history.get_distinct_paths())
    ...
    def search_in_files(self, string_to_search, rootdir=None, suffix=None, subfolders: bool = True):
        ...
        self.search_history.insert(string_to_search, rootdir, self.start_time[0], self.end_time[0], suffix)
        self.folder_entry.set_completion_list(self.search_history.get_distinct_paths())
        self.extension_entry.set_completion_list(self.search_history.get_distinct_extensions())
        self.search_entry.set_completion_list(self.search_history.get_distinct_keywords())
        try:
